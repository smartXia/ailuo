...
import { page, remove, removeBatch,syncWxUser, resetPassword } from "@/api/module/user";
...
export default {
  ...
  setup() {
    ...
    const syncWxUser1 = () => {
       modal.confirm({
        title: '您是否确定要手动同步微信用户?',
        okText: '确定',
        cancelText: '取消',
        onOk() {
          message.loading({ content: "请求中..." });
          syncWxUser().then((response) => {
            if(response.success){
              message.success({content: "成功", key: "", duration: 1}).then(() => 
                tableRef.value.reload()
              )
            }else{
              message.error({content: "失败", key: "", duration: 1})
            }
          })
        }
      });
    ...
    const toolbar = [
      ...
      { label: "手动同步", event: function () { syncWxUser1() }},
    ];
