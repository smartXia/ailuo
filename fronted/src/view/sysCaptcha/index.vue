<template>
  <page-layout>
    ...
    <send :visible="state.visibleSend" @close="closeSend"></send>
...
import { page, remove, removeBatch } from "@/api/module/sysCaptcha";
...
export default {
  ...
  setup() {
    ...
    const columns = [
      ...
      { dataIndex: "captcha", key: "captcha", title: "验证码" },
      { dataIndex: "createTime", key: "createTime", title: "创建时间" },
      { dataIndex: "expired", key: "expired", title: "过期时间(分钟)" },
    ];
    ...
    const toolbar = [
      ...
      { label: "发送测试", event: function () { state.visibleSend = true } },
    ...
    const closeSend = function () {
      state.closeSend = false
      tableRef.value.reload()
    }
