...
public class AliPayServiceImpl extends ServiceImpl<AliPayRepositoryMapper, ToolAlipayConfig> implements IAliPayService {

    @Resource
    AliPayRepositoryMapper alipayRepository;

    @Override
    @Cacheable(key = "'config'")
    public ToolAlipayConfig find() {
        return alipayRepository.selectToolAlipayConfigById(1L);
    }

    @Override
    @CachePut(key = "'config'")
    @Transactional(rollbackFor = Exception.class)
    public ToolAlipayConfig config(ToolAlipayConfig ToolAlipayConfig) {
        if (ToolAlipayConfig.getConfigId() != null) {
            ToolAlipayConfig t = alipayRepository.selectToolAlipayConfigById(ToolAlipayConfig.getConfigId());
            if (t.getConfigId() != null) {
                alipayRepository.updateToolAlipayConfig(ToolAlipayConfig);
            }
            else {
                alipayRepository.insertToolAlipayConfig(ToolAlipayConfig);
            }
        }
        else {
            alipayRepository.insertToolAlipayConfig(ToolAlipayConfig);
